Stop Watch (nothing todo with any other app)

use github workflows for building Stopwatch app

Save changes to changes.txt (create if not exists)

always use same keystore for each app made via github workfloes so it can update correctly without requiring uninstallation
...
its already built but the release install issue needs resolving it only like 2.0 mb abit too small and failed to install on android (i dont want you to attempt to build locally only will bhuild on github via workflows!
